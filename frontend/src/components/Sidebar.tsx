...
export default function Sidebar() {
    ...
    // Fetch notifications when league changes
    useEffect(() => {
        if (!activeLeague || !session) return
        fetch(`${API}/leagues/${activeLeague.id}/notifications`, { headers })
            .then(r => r.json())
    ...
    const markAllRead = async () => {
        ...
        await fetch(`${API}/leagues/${activeLeague.id}/notifications/read-all`, {
    ...
    const markRead = async (id: string) => {
        if (!activeLeague) return
        await fetch(`${API}/leagues/${activeLeague.id}/notifications/read`, {
            method: 'PATCH',
            headers,
    ...
    return (
        <div style={styles.sidebar}>
          <style>{`
              ...
              .bell-btn:hover .scroll-icon {
                  transform: scale(1.2) rotate(-8deg);
                  color: var(--accent);
              }
              .scroll-unread .scroll-icon {
                  color: var(--accent);
              ...
              .scroll-pop .scroll-icon {
                  animation: scrollPop 0.5s ease forwards !important;
              }
          `}</style>
